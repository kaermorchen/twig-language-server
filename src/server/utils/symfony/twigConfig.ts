import {
  ...
  parseDockerComposeExecCommand,
} from '../docker/command';
import { getVolumeBindingsForService } from '../docker/config';
...
export async function getTemplatePathMappingsFromSymfony(
  phpBinConsoleCommand: string,
): Promise<TemplatePathMapping[]> {
  ...
  const dockerComposeCommandParsed = parseDockerComposeExecCommand(
    phpBinConsoleCommand as DockerComposeExecCommand<string>,
  );
  ...
  const bindings = await getVolumeBindingsForService(
    dockerComposeCommandPart,
    service,
  );

  const convertAllBindings = (path: string) => {
    return bindings.reduce(
      (path, { source, target }) => path.replace(target, source),
      path,
    );
  };
  ...
  return [
    ...mappings,
    { directory: convertAllBindings(twigConfig.default_path), namespace: '' },
  ];
}
